AFRAME.registerComponent('extended-wasd-controls', {
	...
	schema: 
	{
		...
		*/
		...
		lookDownKey:     {type: 'string', default: "G"},
  		...
  		maxLookAngle:   {type: 'number',  default: 40},
      maxTiltAngle:   {type: 'number',  default: 40},
	...
	init: function()
	{
		...
		this.rotatePercent = new THREE.Vector3(0,0,0);
		// y = turn angle
		// x = look angle
	...
	tick: function (time, timeDelta) 
	{
		...
		let tiltAmount = (timeDelta/1000) * THREE.Math.degToRad(this.data.lookSpeed);
		let maxLookAngle = THREE.Math.degToRad(this.data.maxLookAngle);
    let maxTurnAngle = THREE.Math.degToRad(this.data.maxLookAngle);
    let maxTiltAngle = THREE.Math.degToRad(this.data.maxTiltAngle);
    ...
    let totalTiltAngle = 0;
      ...
      if ( this.data.lookEnabled )
			{
				this.tiltAngle += this.rotatePercent.z * tiltAmount;
				this.el.object3D.rotation.z = this.tiltAngle;
			}
        ...
        if (this.tiltAngle > maxTiltAngle)
					this.tiltAngle = maxTiltAngle;
				if (this.tiltAngle < -maxTiltAngle)
					this.tiltAngle = -maxTiltAngle;
			}
		}
    ...
    if (resetdrone == true) {
      this.rotatePercent.x = 0;
      this.rotatePercent.z = 0;
      this.lookAngle = 0;
      this.tiltAngle = 0;
